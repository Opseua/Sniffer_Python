...
try {
    async function run() {
        ...
        async function sendGet(inf) {
            ...
            } catch (e) { let m = await regexE({ 'e': e }); console.log(m.res) }; return ret
        }
        ...
        // -------------------------------------------------------------------------------------------------
        let sockReq = net.createServer((socket) => { // ########### REQUEST
            try {
        ...
        let sockRes = net.createServer((socket) => { // ########### RESPONSE
            try {
                let g = ''; socket.on('data', async (chunk) => {
                    g += chunk.toString(); if (g.endsWith('#fim#')) {
                        g = Buffer.from(g.split("#fim#")[0], 'base64').toString('utf-8');
...
catch (e) { let m = await regexE({ 'e': e }); console.log(m.res) }
