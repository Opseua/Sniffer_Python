...
async function addressParseGoogleMaps(inf = {}) {
    ...
    try {
        ...
        if (texto.includes('https://maps.app.goo.gl/')) {
            let retApi = await api({
                method: 'POST', url: urlServer, maxConnect: 5, headers: { raw: true, }, body: JSON.stringify({ fun: [{ securityPass, retInf: true, name: 'api', par: { method: 'GET', url: texto, maxConnect: 5, }, },], }),
            }); if (!retApi.ret) { ret['msg'] = `ADDRESS PARSE GOOGLE MAPS: ERRO | ${retApi.ret}`; alertConsole(`${ret.msg}`); return ret; } retApi = JSON.parse(retApi.res.body).res.body;
