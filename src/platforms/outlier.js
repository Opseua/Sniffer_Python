...
// infOutlier = { e, 'platform': platform, 'url': `${platform}/home`, 'body': inf.body }
...
async function outlier(inf) {
    ...
    try {
        ...
        if ((inf.url == `${platform}${urlCurrent}`)) {
            logConsole({ e, ee, 'write': true, 'msg': `#### ${platform} | ${urlCurrent}` });
            ...
            for (let [index, value] of gO.inf[platform].log.entries()) {
                if (judgeId == value.judgeId) {
                    ...
                    retLog = await log({ e, 'folder': `${pathLogPlataform}`, 'path': `SEND_LINTER-${hitApp}.txt`, 'text': inf.body });
        ...
        if ((inf.url == `${platform}${urlCurrent}`)) {
            ...
            for (let [index, value] of gO.inf[platform].log.entries()) {
                let hitApp = value.hitApp;
                retLog = await log({ e, 'folder': `${pathLogPlataform}`, 'path': `GET_LINTER-${hitApp}.txt`, 'text': inf.body });
                gO.inf[platform].log[index]['lastLinters'] = gO.inf[platform].log[index].lastLinters ?? []; gO.inf[platform].log[index].lastLinters.push(retLog.res);
                break
            }
        }
        ...
        if ((inf.url == `${platform}${urlCurrent}`)) {
            ...
            for (let [index, value] of gO.inf[platform].log.entries()) {
                if (judgeId == value.judgeId) {
                    let hitApp = value.hitApp
                    retLog = await log({ e, 'folder': `${pathLogPlataform}`, 'path': `SEND_${hitApp}.txt`, 'text': inf.body });
                    gO.inf[platform].log[index]['lastLinters'] = gO.inf[platform].log[index].lastLinters ?? []; gO.inf[platform].log[index].lastLinters.push(retLog.res);
                    ...
                    for (let [index1, value1] of gO.inf[platform].log[index].lastLinters.entries()) {
                        pathNew = value1; pathNew = pathNew.substring(pathNew.lastIndexOf('/') + 1); pathNew = value1.replace(pathNew, `OK/${pathNew}`); await file({ e, 'action': 'change', 'path': value1, 'pathNew': pathNew })
                    }
                    retConfigStorage = await configStorage({ e, 'path': `./log/${pathLogPlataform}/${time1}/#_DIA_#.json`, 'functionLocal': false, 'action': 'get', 'key': `${platform}` });
                    ...
                    retConfigStorage = await configStorage({ e, 'path': `./log/${pathLogPlataform}/${time1}/#_DIA_#.json`, 'functionLocal': false, 'action': 'set', 'key': `${platform}`, 'value': json });
                    retConfigStorage = await configStorage({ e, 'path': `./log/${pathLogPlataform}/MES_${time.mon}_${time.monNam}/#_MES_#.json`, 'functionLocal': false, 'action': 'set', 'key': `DIA_${time.day}`, 'value': json.inf });
                    retConfigStorage = await configStorage({ e, 'path': `./log/${pathLogPlataform}/MES_${time.mon}_${time.monNam}/#_MES_#.json`, 'functionLocal': false, 'action': 'get', 'key': `*` });
