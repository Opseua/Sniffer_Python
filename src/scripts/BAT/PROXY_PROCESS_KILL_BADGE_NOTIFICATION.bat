...
rem PROXY → RESETAR TODOS (PROXY → OFF EM TODOS OS CASOS)
if not "!arg1!"=="!arg1:PROXY_=!" (
	...
	rem → ENCERRAR SERVIDOR HTTP PAC FILE
	powershell -Command "Get-CimInstance Win32_Process | Where-Object { $_.CommandLine -like '*serverPacFile*' } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force }"
)
