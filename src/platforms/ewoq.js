...
async function ewoq(inf) {
    ...
    try {
        ...
        if ((inf.url == `${platform}/home`)) {
            ...
            gO.inf[platform] = {}; gO.inf[platform]['log'] = []; gO.inf[platform]['token'] = {}; await csf([gO.inf]);
        ...
        if ((inf.url == `${platform}/GetTemplate_[1-SEND]`)) {
            ...
            let tk = JSON.parse(inf.body)['1']; gO.inf[platform].token['lastToken'] = tk; gO.inf[platform].token[tk] = false
        }
        ...
        if ((inf.url == `${platform}/GetNewTasks`)) {
            ...
            if (body['1']) {
                let id = body['1'][0]['1']['1'].replace(/[^a-zA-Z0-9]/g, ''); let retRegex = regex({ 'pattern': '":"locale","(.*?)"', 'text': inf.body })
                let addGet = {
        ...
        if ((inf.url == `${platform}/SubmitFeedback`)) {
            ...
            let json, body = JSON.parse(inf.body); let id = body['6']['1'].replace(/[^a-zA-Z0-9]/g, '');
            for (let [index, value] of gO.inf[platform].log.entries()) {
                if (id == value.id) {
                    ...
                    if (gO.inf[platform].token.path) {
                        retFile = await file({
                            'action': 'change', 'path': gO.inf[platform].token.path, 'pathNew': gO.inf[platform].token.path.replace(`DIA_${time.day}/`, `DIA_${time.day}/OK/`)
